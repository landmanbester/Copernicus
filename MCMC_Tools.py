class MCMC_diagnostics(object):
    ...
    """
    def __init__(self, M, T, D, savename):
        ...
        self.n = self.T  # /2
        self.samps = zeros([M, self.n,
                            self.D])  # array to store samps (one page for each chain with each column holding samples of a parameter)
        self.X = zeros([self.M * self.n, self.D])
        tmpstr = ['0', '1', '2', '3', '4']
        i = 0
        for tmp in tmpstr:
            holder = load(savename + tmp + '.npz')
